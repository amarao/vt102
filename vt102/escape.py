...
"""Reverse linefeed: Moves cursor up one line in same column. If cursor
is at top margin, screen performs scroll-down."""
RLF = 0x49
...
character set, and origin mode selection (see :data:`DECRC`)."""
DECSC = 0x37

"""Restores previously saved cursor position, character attribute
(graphic rendition), character set, and origin mode selection. If none
were saved, the cursor moves to home position."""
DECRC = 0x38

"""Moves cursor up n lines in same column. Cursor stops at top margin."""
CUU = 0x41

"""Moves cursor down n lines in same column. Cursor stops at bottom
margin."""
CUD = 0x42
...
left. This creates a space character at right margin."""
DCH = 0x50

"""Inserts n lines at line with cursor. Lines displayed below cursor
...
"""Selects top and bottom margins, defining the scrolling region. Pt
is line number of first line in the scrolling region. Pb is line number
of bottom line. If arguments are not selected, the complete screen is
used (no margins)."""
DECSTBM = 0x72

"""Selects insert mode. New display characters move old display
characters to the right. Characters moved past the right margin are
lost."""
IRMI = 0x68
